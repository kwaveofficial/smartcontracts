...
contract KWave is ERC20, Ownable {
    ...
    constructor(address[] memory recipients, uint256[] memory amounts, address ownerAddress) ERC20("K-Wave", "WAVT") Ownable(ownerAddress) {
    ...
    function getLockedAmount(address user) public view returns (uint256) {
        if (block.timestamp < lockedTokens[user].unlockTime) {
            return lockedTokens[user].amount;
        }
        return 0;
    }
    ...
    function transfer(address to, uint256 amount) public override returns (bool) {
        ...
        require(balanceOf(msg.sender) - amount >= locked, "Trying to transfer locked tokens");
        return super.transfer(to, amount);
    }

    function transferFrom(address from, address to, uint256 amount) public override returns (bool) {
        uint256 locked = getLockedAmount(from);
        require(balanceOf(from) - amount >= locked, "Trying to transfer locked tokens");
        return super.transferFrom(from, to, amount);
    }

    function stake(uint256 amount, uint256 duration) external {
        require(amount > 0, "Stake amount must be greater than 0");
        require(duration >= stakeConfigInfo.minStakeDuration, "Duration below minimum stake period");
        require(duration <= stakeConfigInfo.maxStakeDuration, "Duration exceeds maximum stake period");
        _transfer(msg.sender, address(this), amount);

        stakings[msg.sender].push(StakeInfo({
            amount: amount,
    ...
    function claimStake(uint256 index) external {
        ...
        require(block.timestamp >= stakeInfo.startTime + stakeInfo.duration, "Not yet unlocked");

        uint256 reward = (stakeInfo.amount * interestRatePerYearBP * stakeInfo.duration) / (365 days * 10000);
    ...
    function getStakeConfigInfo() external view returns (uint256 minStakeDuration, uint256 maxStakeDuration) {
        return (stakeConfigInfo.minStakeDuration, stakeConfigInfo.maxStakeDuration);
    }
    ...
    function setStakeDurationLimits(uint256 _min, uint256 _max) external onlyOwner {
    ...
    // =============================== utils ===============================
    function parseInterestRate(string memory s) internal pure returns (uint256) {
